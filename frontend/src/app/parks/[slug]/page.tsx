...
export default function SingleParkPage() {
  ...
  const shortParkName = park.name.replace(" National Park", "");
  ...
  return (
    <main className="min-h-screen max-w-5xl mx-auto px-6 py-12 space-y-16">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        {/* Consistent Sidebar Permit Booking Card */}
        <div className="space-y-6">
          <div className="bg-zinc-950 border border-orange-500/30 p-6 md:p-7 rounded-3xl space-y-6 shadow-2xl sticky top-24">
            <div className="space-y-2">
              <span className="text-[10px] font-mono uppercase font-extrabold text-orange-400 bg-orange-500/10 border border-orange-500/30 px-3 py-1 rounded-full inline-block">
                Official Permit Booking
              </span>
              <h3 className="text-2xl font-black text-white tracking-tight leading-tight">
                Reserve {shortParkName}
              </h3>
            ...
            <div className="space-y-3 text-xs border-y border-white/10 py-4 text-zinc-300 font-light">
              <p className="flex items-start gap-2.5">
                <Calendar className="w-4 h-4 text-orange-500 flex-shrink-0 mt-0.5" />
                <span><strong>Season:</strong> {park.best_season}</span>
              </p>
              <p className="flex items-center gap-2.5">
                <Clock className="w-4 h-4 text-orange-500 flex-shrink-0" />
                <span><strong>Slots:</strong> Morning & Evening Safaris</span>
              </p>
            </div>
            ...
            <div className="space-y-3 pt-1">
              <Link
                ...
                className="w-full bg-orange-500 hover:bg-orange-400 text-black font-extrabold py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs shadow-xl shadow-orange-500/20 active:scale-95"
              >
                <Compass className="w-4 h-4 flex-shrink-0" /> Reserve {shortParkName} Permit &rarr;
              </Link>

              <Link
                href="/custom-package"
                onClick={() => triggerHaptic(12)}
                className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs border border-white/10 active:scale-95 text-center"
              >
                Custom {shortParkName} Package
              </Link>
              ...
              <a
                ...
                onClick={() => triggerHaptic(12)}
                className="w-full bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 font-bold py-3 px-4 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs border border-emerald-500/30 active:scale-95"
              >
                <MessageSquare className="w-4 h-4 flex-shrink-0" /> WhatsApp Inquiry (+91 9425331205)
              </a>
            </div>
          </div>
        </div>
