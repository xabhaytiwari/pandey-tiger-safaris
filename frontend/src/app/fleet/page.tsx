  ...
  return (
    <main className="min-h-screen max-w-6xl mx-auto px-6 py-12">
      <div className="space-y-4 text-center max-w-3xl mx-auto mb-16">
        <span className="text-xs font-mono uppercase tracking-widest text-amber-400">Transport Fleet</span>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {cars.map((car) => (
          <div key={car.id} className="bg-zinc-900/40 border border-white/10 rounded-3xl p-6 backdrop-blur-md hover:border-amber-500/40 transition-all flex flex-col justify-between">
            <div className="space-y-4">
              <img src={car.image_url} alt={car.name} className="w-full h-48 object-cover rounded-2xl" />
              <div>
                <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">{car.category}</span>
              ...
              {car.capacity > 0 && (
                <div className="text-xs text-zinc-500 flex items-center gap-1.5 pt-2 border-t border-white/5">
                  <Users className="w-3.5 h-3.5 text-amber-400" /> Capacity: {car.capacity} Passengers
                </div>
              )}
            </div>
